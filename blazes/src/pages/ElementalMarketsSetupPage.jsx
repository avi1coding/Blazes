...
export default function ElementalMarketsSetupPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const { kit, user } = location.state || {};

  const [gameName, setGameName] = useState(`${kit?.title || 'Markets'} Trading`);
  ...
  const handleCreateGame = async () => {
    ...
    try {
      ...
      const res = await fetch(`${base}/api/games/create`, {
        ...
        body: JSON.stringify({ hostId: user.id, kitId: kit.id, gameCode, gameMode: 'elemental_markets', settings }),
      });
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6">
        <div className="text-center mb-8 sm:mb-10 md:mb-12">
          ...
          <p className="text-base sm:text-lg text-gray-600">Configure your 'Elemental Markets' game.</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 md:p-8">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-400">
                Each player starts with $1000. Earn cash by answering questions, trade 6 elemental
                stocks (Fire, Water, Earth, Air, Lightning, Ice). Highest portfolio value wins.
              </p>
            </div>
          ...
          <div className="mt-10 flex justify-end gap-4">
            ...
            <button onClick={handleCreateGame} disabled={loading}
              className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-lg font-bold hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-wait">
