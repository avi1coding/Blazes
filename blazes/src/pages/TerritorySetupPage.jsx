...
import { Play, Clock, Grid3x3 } from 'lucide-react';
import HostPlaysToggle from '../components/HostPlaysToggle';

/**
 * Territory runs until the teacher ends it or the clock they set here runs
 * out, same shape as the game length control everywhere else in the app:
 * a plain number of minutes, not a dropdown, with the option to extend
 * mid-game from the lobby/gameplay screen.
 */
export default function TerritorySetupPage() {
  ...
  const [gameName, setGameName] = useState(`${kit?.title || 'Blazes'} Territory`);
  ...
  const handleCreateGame = async () => {
    ...
    try {
      ...
      const res = await fetch(`${base}/api/games/create`, {
        ...
        body: JSON.stringify({
          hostId: user.id, kitId: kit.id, gameCode, gameMode: 'territory',
          settings: {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto py-12 px-4 sm:px-6">
        <div className="text-center mb-8">
          ...
          <p className="text-base sm:text-lg text-gray-600">Configure your Territory game.</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 md:p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-black text-gray-900">Territory</h2>
          ...
          <p className="text-sm text-gray-600 bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6">
            A shared grid, everyone answering at once. Every correct answer claims the tile next to your
            last one, so your color spreads across the board on its own. Left alone, tiles fade back to
            open ground, so there's always more to claim.
          </p>
